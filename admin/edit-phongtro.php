...
<body>
    <div class="modal fade" id="edit_phongtro<?php echo $row['motel_id']; ?>" data-bs-backdrop="static" data-bs-keyboard="false" tabindex="-1" role="dialog" aria-labelledby="" aria-hidden="true">
        <div class="modal-dialog modal-xl" role="document">
        <?php
            include("../connect.php");
            $sql = "SELECT * FROM motel m INNER JOIN user u ON m.user_id = u.id WHERE motel_id='".$row['motel_id']."'" ;
            $query = mysqli_query($connect, $sql);
            // $row = mysqli_fetch_array($query);
        ?>
        <form action="edit-phongtro.php?motel_id=<?php echo $row['motel_id']; ?>" method="POST" enctype="multipart/form-data">
            ...
            <div class="modal-footer">
                ...
                <button type="submit" class="btn btn-primary" name="edit_phongtro">Lưu</button>
            </div>
            </div>
...
<?php
    ...
    {   
        ...
        if(file_exists("../uploads/".$img_name)) {
            ...
            if($query_run){
                move_uploaded_file($_FILES['images']['tmp_name'], '../uploads/'.$img_name);
